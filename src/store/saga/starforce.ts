...
export function* starforceSaga() {
  yield all([fork(watchInput), fork(watchButton)]);
}
...
function* watchButton() {
  while (true) {
    ...
    const errorMessage: { [index: string]: string } = yield select(
      (state) => state.starforce.errorMessage
    );

    if (
      Object.keys(errorMessage).filter((v) => errorMessage[v] !== "").length ===
      0
    ) {
...
// 시뮬레이션 함수
function* calc() {
  const { equipLevel, spareCost, currentStar, targetStar } = yield select(
    (state) => state.starforce.equipInfo
  );
  const { simNum, safeguardArr, starcatchArr, event, discount } = yield select(
    (state) => state.starforce.detail
  );
  const option: Option = {
    equipInfo: {
  ...
  do {
    try {
      const result: {
        resultArr: { cost: number; destroyed: number }[];
...
function* handleErrorMessage() {
  ...
  if (currentStar !== undefined) {
    if (currentStar < 0) {
      ...
      currentStar >= getReachableStar(equipLevel)
    ) {
      message.currentStar = `${getReachableStar(
        equipLevel
      )} 미만의 수치를 입력해주세요.`;
  ...
  if (targetStar === undefined) {
    ...
    equipLevel !== undefined &&
    targetStar > getReachableStar(equipLevel)
  ) {
    message.targetStar = `${getReachableStar(
      equipLevel
    )} 이하의 수치를 입력해주세요.`;
